...
if __name__ == '__main__':
	...
	while repeat:
		command = input('Введите команду (s,r,q; s - send mail, r - recieve mail, q - выход) ')
		if command == 'q' :
			repeat = False
			break
		elif command == 's':
			...
			print('Отправляем почту для:', rcpt, ' Заголовок:', sbjct, ' сообщение:', body)
			send_mail_result = mailconnector.send_mail(LOGIN, rcpt, sbjct, body)
			print('Результат отправки:', send_mail_result)
		elif command == 'r':
			print('Будем получать почту')
			recieve_mail_result = mailconnector.recieve_mail(HEADER)
